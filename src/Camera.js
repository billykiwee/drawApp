              ...
              return (
                <>
                  <div className="w-100 h-100 grid absolute overflow-hidden zi-2">
                    {LoadImag && (
                      <div
                        className="display justify-c w-100 gap zi-2"
                        style={{ height: "10vh" }}
                      >
                        <div
                          className="display white border-r-04"
                          style={{ opacity: Lock && 0.4 }}
                        >
                          <ButtonControl
                            onClick={(e) => !Lock && setPosition(Position - 90)}
                            className={Lock && "opacity"}
                          >
                            <svg
                              width={22}
                          ...
                          <ButtonControl
                            onClick={(e) =>
                              !Lock &&
                              setZoom(
                                Zoom !== widthMin ? Zoom - widthStep : Zoom
                              )
                            }
                          >
                            <svg
                          ...
                          <ButtonControl
                            onClick={(e) => !Lock && setZoom(Zoom + widthStep)}
                          >
                            <svg
                              width={22}
                              xmlns="http://www.w3.org/2000/svg"
                        ...
                        <button
                          className={
                            (Lock ? "red" : "white") + " display w-3 h-3"
                          }
                          onClick={(e) =>
                    ...
                    <div
                      className="display justify-c select_none"
                      style={{ height: "80vh" }}
                    >
                    ...
                    <div
                      className="display justify-c w-100 gap zi-2 bottom-0 fixed"
                      style={{ height: "10vh" }}
                    >
                      <ButtonControl
                        style={{
                      ...
                      >
                        ...
                        {status === "recording" && (
                          <svg
                            ...
                            xmlns="http://www.w3.org/2000/svg"
                          >
                            <circle cx="5" cy="5" r="5" fill="#EB3B5A" />
                          </svg>
                        )}
                      </ButtonControl>
                      <div
                        className="display justify-c white h-3 border-r-04 p-lr-04"
                        style={{
                          pointerEvents: Lock && "none",
                          opacity: Lock && 0.4,
                        }}
...
function ButtonControl({ children, onClick, className, style }) {
  return (
    <div className="display">
      <button
        type="button"
        className={"click white h-3 w-3 hover-white shadow " + className}
        onClick={onClick}
        style={style}
      >
        <span className="display">{children}</span>
      </button>
    </div>
  );
}
